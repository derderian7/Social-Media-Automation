...
use App\Http\Controllers\PostsCronController;
...
class publishIGpostCommand extends Command
{
    ...
    {
        $posts = PostsCronController::get_instagram_posts(true);
        if(isset($posts['media']) && !empty($posts['media'])){
            ...
            foreach ($posts['media'] as $image_post){
                ...
                if(!isset($pages_insta_to_face[$igUserId]['access_token'])) {
                    ...
                    PostsCronController::update_instagram_post($post_request, $post_id_from_database);
                    continue;
                }
                ...
                if(isset($graphNode) && $graphNode !=''){
                    ...
                    if(isset($readable_result['id']) && !empty($readable_result['id'])){
                        echo 'the media ' . $readable_result['id'] . ' was created '; echo '<br />';
                        ...
                        echo '<pre>'; print_r('error creating media container'); echo '</pre>';
